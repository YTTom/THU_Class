...
select_sql="select * from LoRa where `timestamp`= %s and `sensor`='A'"
cursor.execute(select_sql,date)
result=cursor.fetchall()
conn.commit()
a = 'A'
if result==():
    insert_sql="insert into LoRa(id,sensor,temp,hum,PM25,PM10,timestamp) values (null,'A',%s,%s,%s,%s,%s)"
    data= temp,hum,PM25,PM10,date
    cursor.execute(insert_sql,data)
    cursor.execute("""
       UPDATE pc_lora_rt
       SET temp=%s,hum=%s,PM25=%s,PM10=%s,timestamp=%s
       WHERE sensor='A'
       """, (temp,hum,PM25,PM10,date))
    conn.commit()
else:
    conn.close()
#print(date)
